...
const StudentDetailsFakerDialogComponent = (props) => {
    ...
    const [courseIDIds, setcourseIDIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("courseDetails")
                        .find({ query: { $select: ["_id","courseID"] } })
                        .then((data) => {
                        setcourseIDIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = studentDetailsFakerFactory(props.user,fakerCount,courseIDIds);
